...
public final class GregoriusDrugworksMatProperties {
    ...
    private static void ensureAddonMaterialForms() {
        for (Field field : GregoriusDrugworksMaterials.class.getDeclaredFields()) {
            if (!Modifier.isStatic(field.getModifiers()) || field.getType() != Material.class) {
                continue;
            }
            try {
                ...
                if (material.hasFlag(GENERATE_PLATE) && !material.hasProperty(PropertyKey.DUST)) {
                    material.setProperty(PropertyKey.DUST, new DustProperty());
                }
                if (material.hasProperty(PropertyKey.DUST)) {
                    material.addFlags(FORCE_GENERATE_BLOCK);
                }
            } catch (IllegalAccessException exception) {
                throw new IllegalStateException("Failed to normalize forms for addon material field "
                        + field.getName(), exception);
            }
        }
    }
